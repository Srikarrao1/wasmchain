...
for running end-to-end tests on Anryton.
It is used for testing chain upgrades,
as it allows for initializing multiple Anryton chains with different versions.
...
# version(s) of initial anryton node(s) that will be upgraded, tag e.g. 'v9.1.0'
# to use multiple upgrades separate the versions with a forward slash, e.g.
# 'v10.0.1/v11.0.0-rc1'
INITIAL_VERSION

# version of upgraded anryton node that will replace the initial node, tag e.g.
# 'v10.0.0'
TARGET_VERSION

# mount point for the upgraded node container, to mount new node version to
# previous node state folder. By default this is './build/.anrytond:/root/.anrytond'
# More info at https://docs.docker.com/engine/reference/builder/#volume
MOUNT_PATH

# '--chain-id' anryton cli parameter, used to start nodes with a specific
# chain-id and submit proposals
# By default this is 'anryton_9000-1'
CHAIN_ID
```
...
1. Build a docker image for the anryton target version
...
the test suite exports `/.anrytond` folder from the docker container
...
This package is decoupled from depending on the Anryton codebase.
It initializes the chains for testing via Docker.  
As a result, the test suite may provide the
desired Anryton version to Docker containers during the initialization.
This design allows for the opportunity of testing chain upgrades
by providing an older Anryton version to the container,
...
to run different versions of anryton containers,
...
will be pulled from [DockerHub](https://hub.docker.com/r/tharsishq/anryton/tags).
If it is not specified,
the test suite retrieves the second-to-last upgrade version
from the local codebase (in the `anryton/app/upgrades` folder)
...
```log
ok  	github.com/anryton/anryton/v9/tests/e2e	174.137s.
```
...
Error:  Received unexpected error:
        can't start anryton node, container exit code: 2
        ...
        main.appCreator.newApp({{{0x3399b40, 0xc000ec1db8}, {0x33ac0f8, 0xc00...
            github.com/anryton/anryton/v10/cmd/anrytond/root.go:243 +0x2ca
        github.com/anryton/ethermint/server.startInProcess(_, {{0x0, 0x0, 0x0},...
            github.com/anryton/ethermint@v0.20.0-rc2/server/start.go:304 +0x9c5
        github.com/anryton/ethermint/server.StartCmd.func2(0xc001620600?, {0xc0...
            github.com/anryton/ethermint@v0.20.0-rc2/server/start.go:123 +0x1ec
        ...
        main.main()
            github.com/anryton/anryton/v10/cmd/anrytond/main.go:20 +0x45
...
```log
CONTAINER ID   IMAGE
9307f5485323   anryton:local    <-- upgraded node
f41c97d6ca21   anryton:v9.0.0   <-- initial node
```
